"""cadogan_sinopec_finally_came_up_with_agent_name — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.cadogan_sinopec_finally_came_up_with_agent_name._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.cadogan_sinopec_finally_came_up_with_agent_name import prompt as prompt_mod
from sea_of_colours.orchestrator_2.harnesses.cadogan_sinopec_finally_came_up_with_agent_name import (
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME"
THINKER_AGENT_LABEL = "CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_SINGLE=1`` disables."""
    return os.environ.get("CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
    ...
    payload = {
        ...
        "extras": {
            ...
            "sub_invocations": (
                [
                    {
                        "label": "CADOGAN_SINOPEC_FINALLY_CAME_UP_WITH_AGENT_NAME_THINK",
                        "kind": "think",
                        "api": "chat",
