...
def plan_orbit_actions(
    ...
    *,
    ...
    # FLEET FIRST for expensive weapons, but SNAP is cheap enough
    # (100 blue + 250 credits) to carry from day 2. A SNAP in the rack
    # on the night a rival finds a pure is the difference between
    # denying their landing and watching them sweep it.
    fleet_ready = cap_used >= 2
    ...
    # SNAP slips through the fleet gate: buy one early if affordable.
    if (
        weapons_enabled
        and not fleet_ready
        and snap_stock < 1
        and blue_total >= snap_blue_cost
        ...
        descriptors.append(
            f"early SNAP (100 blue, denial ready for redsign night)"
        )

    if weapons_enabled and fleet_ready and not _room_for(min(emp_blue_cost, chaff_blue_cost)):
    ...
    elif weapons_enabled and fleet_ready and blue_total > dials.blue_always_build:
    ...
    remaining = weapon_forge.add_procurement(
        ...
        weapons_enabled=weapons_enabled and fleet_ready)
